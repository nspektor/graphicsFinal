...
Returns:
Go through points 2 at a time and call Zdraw_line to add that line
to the screen
====================*/
...
void Zdraw_polygons( struct matrix *polygons, screen s, color c , struct matrix* zbuffer) {
	...
	for( i=0; i < polygons->lastcol-2; i+=3 ) {
		...
		if ( calculate_dot( polygons, i ) < 0 ) {
			Zdraw_line( polygons->m[0][i], polygons->m[1][i], polygons->m[2][i], polygons->m[0][i+1], polygons->m[1][i+1], polygons->m[2][i+1], s, c, zbuffer);
			Zdraw_line( polygons->m[0][i+1], polygons->m[1][i+1], polygons->m[2][i+1], polygons->m[0][i+2], polygons->m[1][i+2], polygons->m[2][i+2], s, c, zbuffer);
			Zdraw_line( polygons->m[0][i+2], polygons->m[1][i+2], polygons->m[2][i+2], polygons->m[0][i], polygons->m[1][i], polygons->m[2][i], s, c, zbuffer);
		}
	}
}
